...
def main():
    ...
    # 加载评分表
    score_table = Table.load(path=join(root, 'calculate.txt'), dtype=object)
    ...
    for organ in ogs:
        ...
        for group, color in zip(config['evaluate.target_group'], ['g', 'y', 'r']):
            ...
            matrix = score_table[samples].data.sum(axis=0)
            ...
            print(f' -> %15s \t {group} \t dice %.2f \t 混淆矩阵 {matrix.tolist()}' % (organ, dice))
