...
class GaussCRF(nn.Module):
    ...
    def __init__(self, conf, shape, nclasses=None, use_gpu=True, device=0):
        ...
        self.CRF = ConvCRF(
            ...
            pyinn=conf['pyinn'], device=device)
    ...
    def forward(self, unary, img, num_iter=5):
        ...
        pos_feats = self.create_position_feats(sdims=self.pos_sdims, bs=bs).to(unary.device)
        col_feats = self.create_colour_feats(
            img, sdims=self.col_sdims, schan=self.col_schan,
            bias=conf['col_feats']['use_bias'], bs=bs
        ).to(unary.device)

        compats = [self.pos_compat.to(unary.device), self.col_compat.to(unary.device)]
...
class MessagePassingCol(object):
    ...
    def __init__(self, feat_list, compat_list, merge, npixels, nclasses,
        ...
        for feats, compat in zip(feat_list, compat_list):
            gaussian = self._create_convolutional_filters(feats).to(self.device)
            if not norm == "none":
    ...
    def _get_norm(self, gaus):
        norm_tensor = torch.ones([1, 1, self.npixels[0], self.npixels[1]]).to(self.device)
        normalization_feats = torch.autograd.Variable(norm_tensor)
        # if self.use_gpu:
        #     normalization_feats = normalization_feats.cuda(self.device)
    ...
    def _compute_gaussian(self, input, gaussian, norm=None):
        ...
        if self.pyinn:
            ...
            input_unfold = F.unfold(input, self.filter_size, 1, self.span).to(self.device)
...
class ConvCRF(nn.Module):
    ...
    def add_pairwise_energies(self, feat_list, compat_list, merge, device):
