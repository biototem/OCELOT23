...
from .conv_crf import GaussCRF
from .conv_crf_config import crf_config, class_nums, device
...
CRF_ARGS = {
    'shape': (config['cropper.patch_size'], ) * 2,
    # 'shape': (7, 7),
}
...
class Net(torch.nn.Module):
    ...
    def forward(self, image: torch.tensor):
        ...
        # 使用 crf
        seg_result = self.crf.forward(unary=seg_result, img=image, num_iter=1)
        ...
        if self.use_auxiliary and self.training:
            ...
            # 使用 crf
            aux_result = self.crf.forward(unary=aux_result, img=image, num_iter=1)
