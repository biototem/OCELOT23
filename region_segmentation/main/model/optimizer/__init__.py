...
from ..net import Net24, Van, VanCrf
...
def optimizer(model: torch.nn.Module) -> optim.Optimizer:
    ...
    elif model.__class__ == VanCrf:
        return van_crf_optimizer(model)
...
def van_crf_optimizer(model: torch.nn.Module) -> optim.Optimizer:
    # print(model)
    ...
    params = [
        ...
        {
            'params': model.auxiliary.parameters(),
            'lr': float(config['model.lr.auxiliary']),
        },
        {
            'params': model.crf.parameters(),
            'lr': float(config['model.lr.crf']),
        },
        {
            'params': model.activation.parameters(),
            'lr': float(config['model.lr.activation']),
        },
    ]
    name = config['model.optimizer']
    if name == 'adam':
