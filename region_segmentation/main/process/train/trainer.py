    ...
    def run(self, dataloader: DataLoader, epoch: int) -> None:
        ...
        with tqdm(dataloader, desc=f'training-{epoch + 1}', file=sys.stdout, disable=not self.verbose) as iterator:
            for images, labels, masks in iterator:
                ...
                masks = torch.stack([masks] * labels.shape[1], dim=1).to(self.device).type(labels.dtype)

                # from utils import PPlot
                # print(images.numpy().transpose(0, 2, 3, 1).shape)
                # PPlot().add(*images.numpy().transpose(0, 2, 3, 1)).show()
                ...
                for predicts in all_predicts:
                    # mask 截断
                    # predicts[~masks] = labels[~masks].type(torch.float32)

                    # 这种策略可能太粗暴了
                    # predicts = predicts * masks
                    # labels = labels * masks
                    # p_loss = self.loss(predicts, labels, masks)
