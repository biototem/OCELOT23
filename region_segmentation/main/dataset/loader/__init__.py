from .image_cache_single import ImageLoader
...
class Loader(object):
    def __init__(self, part_id: str, info: dict, in_memory: bool = False):
        ...
        self.image = image_loader(info['image'], zoom=info['image_zoom'], in_memory=in_memory)
        # 标签加载器
        label_loader = LOADERS[info['label_loader']]
        self.label = label_loader(info['label'], zoom=info['label_zoom'], in_memory=in_memory)
        # 蒙版加载器
        mask_loader = LOADERS[info['mask_loader']]
        self.mask = mask_loader(info['mask'], zoom=info['mask_zoom'], in_memory=in_memory)
