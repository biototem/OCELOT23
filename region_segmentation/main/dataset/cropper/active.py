...
class ActiveCropper(Cropper):
    # 随便定一个 “平均权重”
    MEAN_WEIGHT = sum(config['dataset.active_weight']) / len(config['dataset.active_weight']) / 5
    ...
    def load(self, loader: Loader) -> List[int]:
        ...
        for _, x in magic_iter(
                ...
                unit=self.active_size,
                step=self.active_size,
                margin=(0, 0)
        ):
            for _, y in magic_iter(
                ...
                piece = loader.label({
                    'x': l + x + self.active_size // 2,
                    'y': u + y + self.active_size // 2,
                    'degree': 0,
                    'scaling': 1,
                    'w': self.active_size,
                    'h': self.active_size,
                ...
                for i, p in enumerate(self.weights):
                    sum_p += ((piece == i) * mask.astype(bool)).sum() * p
    ...
    def sample(self, loader: Loader, num: int = -1):
        ...
        # 随机遍历(按权重)
        if num == -1:
            # 本图总权重数 除以 图块像素 再除以 每个像素平均权重
            num = math.ceil(active['max'] / self.patch_size ** 2 / ActiveCropper.MEAN_WEIGHT)
